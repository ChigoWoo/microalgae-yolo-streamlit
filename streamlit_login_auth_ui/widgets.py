...
from streamlit_login_auth_ui.utils import load_lottieurl
from streamlit_login_auth_ui.utils import check_valid_name
...
from streamlit_login_auth_ui.utils import change_passwd
from streamlit_login_auth_ui.utils import check_current_passwd
...
class __login__:
    ...
    def login_widget(self) -> None:
        ...
        if st.session_state['LOGGED_IN'] == False:
            ...
            with del_login.form("Login Form"):
                username = st.text_input("用户名(Username)", placeholder = 'Your unique username')
                password = st.text_input("密码(Password)", placeholder = 'Your password', type = 'password')

                st.markdown("###")
                login_submit_button = st.form_submit_button(label = '登录(Login)')
    ...
    def sign_up_widget(self) -> None:
        ...
        with st.form("Sign Up Form"):
            name_sign_up = st.text_input("姓名(Name) *", placeholder = 'Please enter your name')
            valid_name_check = check_valid_name(name_sign_up)

            email_sign_up = st.text_input("邮箱(Email) *", placeholder = 'Please enter your email')
            valid_email_check = check_valid_email(email_sign_up)
            unique_email_check = check_unique_email(email_sign_up)
            
            username_sign_up = st.text_input("用户名(Username) *", placeholder = 'Enter a unique username')
            unique_username_check = check_unique_usr(username_sign_up)

            password_sign_up = st.text_input("密码(Password) *", placeholder = 'Create a strong password', type = 'password')

            st.markdown("###")
            sign_up_submit_button = st.form_submit_button(label = '注册(Register)')

            if sign_up_submit_button:
                if valid_name_check == False:
                    st.error("请输入合法姓名！(Please enter a valid name!)")

                elif valid_email_check == False:
                    st.error("请输入合法邮箱地址！(Please enter a valid Email!)")
                
                elif unique_email_check == False:
                    st.error("该邮箱已注册！(Email already exists!)")
                
                elif unique_username_check == False:
                    st.error(f'对不起，该用户名已被注册！(Sorry, username {username_sign_up} already exists!)')
                
                elif unique_username_check == None:
                    st.error('用户名不能为空！(Please enter a non - empty Username!)')
    ...
    def forgot_password(self) -> None:
        ...
        with st.form("Forgot Password Form"):
            email_forgot_passwd = st.text_input("邮箱(Email)", placeholder= 'Please enter your email')
            email_exists_check, username_forgot_passwd = check_email_exists(email_forgot_passwd)

            st.markdown("###")
            forgot_passwd_submit_button = st.form_submit_button(label = '获取密码(Get Password)')

            if forgot_passwd_submit_button:
                if email_exists_check == False:
                    st.error("该邮箱尚未注册！(Email ID not registered with us!)")
                ...
                if email_exists_check == True:
                    ...
                    st.success("已发送密码至您的邮箱！(Secure Password Sent Successfully!)")
    ...
    def reset_password(self) -> None:
        ...
        with st.form("Reset Password Form"):
            email_reset_passwd = st.text_input("邮箱(Email)", placeholder= 'Please enter your email')
            email_exists_check, username_reset_passwd = check_email_exists(email_reset_passwd)

            current_passwd = st.text_input("临时密码(Temporary Password)", placeholder= 'Please enter the password you received in the email')
            current_passwd_check = check_current_passwd(email_reset_passwd, current_passwd)

            new_passwd = st.text_input("新密码(New Password)", placeholder= 'Please enter a new, strong password', type = 'password')

            new_passwd_1 = st.text_input("再次输入新密码(Re - Enter New Password)", placeholder= 'Please re- enter the new password', type = 'password')

            st.markdown("###")
            reset_passwd_submit_button = st.form_submit_button(label = '重置密码(Reset Password)')

            if reset_passwd_submit_button:
                if email_exists_check == False:
                    st.error("该邮箱不存在！(Email does not exist!)")

                elif current_passwd_check == False:
                    st.error("临时密码不正确！(Incorrect temporary password!)")

                elif new_passwd != new_passwd_1:
                    st.error("密码不一致！(Passwords don't match!)")
            
                if email_exists_check == True:
                    if current_passwd_check == True:
                        change_passwd(email_reset_passwd, new_passwd)
                        st.success("密码重置成功！(Password Reset Successfully!)")
    ...
    def nav_sidebar(self):
        ...
        with main_page_sidebar:
            selected_option = option_menu(
                ...
                options = ['登录(Login)', '注册(Sign Up)', '忘记密码？(Forgot Password?)', '重置密码(Reset Password)'],
                styles = {
    ...
    def build_login_ui(self):
        ...
        if selected_option == '注册(Sign Up)':
            self.sign_up_widget()

        if selected_option == '忘记密码？(Forgot Password?)':
            self.forgot_password()

        if selected_option == '重置密码(Reset Password)':
            self.reset_password()
